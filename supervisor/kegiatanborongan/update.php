...
{
    $id = $_GET["id"];
    $_data_kegiatan = find("tb_kegiatan", array("id" => $id))[0];
    $data_detail_kegiatan = find("tb_detail_kegiatan", array("id_kegiatan" => $id));
    $_data_detail_kegiatan = array();
    foreach($data_detail_kegiatan as $detail_kegiatan) {
        $_data_detail_kegiatan[$detail_kegiatan["id_tarif_borongan"]] = $detail_kegiatan["jumlah"];
    }
}
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="borongan" id="borongan" class="form-control">
                                        <?php foreach ($daftar_borongan as $borongan): ?>
                                            <option value="<?=$borongan["id"]?>" <?=$_data_kegiatan["id_borongan"] == $borongan["id"] ? 'selected' : ''?>><?=$borongan["nama"]?></option>
                                        <?php endforeach ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <?php foreach ($daftar_kegiatan as $kegiatan): ?>
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="number" class="form-control" name="<?=$kegiatan["id"]?>" placeholder="<?=$kegiatan["nama"]?>" value="<?=$_data_detail_kegiatan[$kegiatan["id"]]?>">
                                        </div>
                                    </div>
                                <?php endforeach ?>
                            </div>
